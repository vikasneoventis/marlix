...
class Dom implements ConverterInterface
{
     ...
     * @return \DOMElement
     */
    protected function getRootNode(\DOMDocument $document)
    {
     ...
     */
    protected function getChildrenByName(\DOMElement $parent, $name, $asArray = true)
    {
     ...
     * @return array
     */
    protected function collectExternalPaymentMethods(array $rootNode)
    {
        $result = [];
        foreach ($rootNode as $methodNode) {
            $method = $this->getAttribute($methodNode, 'id');
            // TODO: Reinstate this check after resolving issues with scope
//            $ifconfig = $this->getAttribute($methodNode, 'ifconfig');
//            if ($ifconfig && !$this->config->isSetFlag($ifconfig)) {
//                continue; // method is disabled, so skip
//            }
     ...
     * @return string
     */
    protected function getAttribute(\DOMElement $element, $name)
    {
     ...
     * @return array
     */
    protected function collectAllChildValues(\DOMElement $parent)
    {
     ...
     * @return array
     */
    protected function collectChildren(\DOMElement $rootNode, $childName, $idField = 'id')
    {
        $result = [];
     ...
     * @return array
     */
    protected function collectOrderLines(array $rootNode)
    {
        $result = [];
