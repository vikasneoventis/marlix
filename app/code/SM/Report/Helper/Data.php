                    ...
                    if (in_array($searchColumnName, $listFilterDataCanCompare)) {
                        ...
                        if ($is_filter_total_value) {
                            ...
                            elseif ($searchColumnName == 'margin') {
                                $collection->getSelect()->having('ROUND ((revenue - total_cost) / revenue, 2)' . $typeSearch . '?', $searchValue);
                            }
                            ...
                            elseif ($searchColumnName == 'discount_percent') {
                                $collection->getSelect()->orhaving('ROUND ((-discount_amount / (-discount_amount+base_row_total_product))*100, 2)' . $typeSearch . '?', $searchValue);
                                $collection->getSelect()->orhaving('ROUND ((discount_amount / (discount_amount+base_row_total_product))*100, 2)' . $typeSearch . '?', $searchValue);
                            }
                            ...
                            elseif ($searchColumnName == 'return_percent') {
                                $collection->getSelect()->having('ROUND ((total_refund_items / total_cart_size)*100, 2) ' . $typeSearch . '?', $searchValue);
                            }
