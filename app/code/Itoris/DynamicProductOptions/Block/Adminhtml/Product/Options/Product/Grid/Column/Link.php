...
class Link extends \Magento\Backend\Block\Widget\Grid\Column\Renderer\Text
{
    ...
    public function render(\Magento\Framework\DataObject $row) {
        return '<a href="#" onclick="itorisDynamicOptions.addProductLinkToItem({productId: '.$row->getId().', name: \''.$this->escapeHtml($row->getName()).'\', price: '.round($row->getPrice(), 2).', price_type: \'fixed\'});return false;">' . $this->escapeHtml(__('select')) . '</a>';
    }
}